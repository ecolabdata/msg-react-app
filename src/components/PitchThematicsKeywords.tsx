...
const OverlappingTitle: React.FC<{txt: string, usedInListPage:boolean, number: number}> = ({txt, usedInListPage, number }) => {
    ...
    return <>
        ...
        <div className={`titleContainer ${!usedInListPage && 'absolute h-0 -top-5 left-4 overflow-visible'}`}>
...
export const PitchThematicsKeywords: React.FC<PitchThematicsKeywordsParams> = ({ usedDescription, usedSecteurs, usedMotsClef, usedInListPage, openPitchContainerFromStart }) => {
    ...
    return (
        <div className={`globalFormContainer ${usedInListPage ? 'addBorder border border-dark-text-action bg-background-form lg:flex-row' : 'justify-around lg:flex-row'}
        ${usedInListPage  && !openPitchContainer  ?'h-10 justify-center items-center' : 'lg:h-[408px]'}
         ...
         transition-all duration-700`}>
            ...
            { usedInListPage && openPitchContainer  &&  
                ...
                <>
                    ...
                    <div className={`leftSideForm ${!usedInListPage ? 'bg-background-form  h-[83%] lg:w-1/2 xl:max-w-[588px] ' : ' lg:w-1/2'} 
                        mb-8 h-[75%] w-full flex justify-center rounded-md  
                        `}>
                        <div className={`projectContainer ${usedInListPage ? 'mt-8 h-fit' : 'h-[150px] items-center w-full -- lg:h-full'}
                        relative  w-[92%] flex flex-col justify-center 
                        lg:mt-0`}>

                            <OverlappingTitle usedInListPage={usedInListPage} number={1} txt="Le projet" />
                                
                            <div className={ `fieldsContainer mt-2 flex flex-col w-full   ${!usedInListPage && 'mt-8'}`}>
                    ...
                    <div className={`rightSideForm ${!usedInListPage ? 'h-[83%] justify-around xl:max-w-[588px]' : ' bg-background-form '}
                        mb-8 h-[75%] w-full flex flex-col items-center justify-between
                        lg:w-1/2
                        `} >
                        
                        <div className={`thematicsContainer rounded-md relative h-[50%] ${!usedInListPage ? 'max-h-[129px] min-h-[129px] w-full flex flex-col items-center bg-background-form ' : 'w-[92%]'}`}>
                        ...
                        <div className={`keyWordsContainer ${!usedInListPage ? 'w-full h-full flex flex-col items-center bg-background-form': 'w-[92%] items-end'}
            ...
            {!usedInListPage &&
                <>
                    ...
                    <div className={`leftSideForm ${!usedInListPage ? 'bg-background-form mt-8 h-[83%] lg:w-1/2 xl:max-w-[588px] ' : ' lg:w-1/2'} 
                        mb-8 h-[75%] w-full flex justify-center rounded-md  
                        `}>
                        <div className={`projectContainer ${usedInListPage ? 'mt-8 h-fit' : 'h-[150px] items-center w-full -- lg:h-full'}
                        relative  w-[92%] flex flex-col justify-center 
