...
export const ActeurPublicResultCard: React.FC<ActeurPublicResultCardProps> = ({
  ...
  const startups = [
    ...(ap.fields['labelled_startups.nom'] || []),
    ...(ap.fields['public_references_startups.nom'] || [])
  ];
  ...
  return (
    ...
    >
      ...
      {countDecp ? (
        <>
          <b style={{ color: cardType.color }}>{countDecp}</b> Marchés passés
        </>
      ) : null}
      {countDecp && countApproch ? ' et ' : null}
      {countApproch ? (
        <>
          <b style={{ color: cardType.color }}>{countApproch}</b> Achats programmés
        </>
      ) : null}
