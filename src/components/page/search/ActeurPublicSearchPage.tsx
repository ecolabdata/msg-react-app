...
export const ActeurPublicResultCard: React.FC<ActeurPublicResultCardProps> = ({ isLoading, ap, cardType }) => {
    ...
    const approchFirst = Object.keys(ap.highlight).filter(key => key.startsWith("approch_content."))
    ...
    return <ResultCard cardType={acheteurPublic} name={public_actor_nom} nameElem={hlPublic_actor_nom} toprow={'Ville / Région'} linkData={{}} slug={slug} isLoading={isLoading}>
        {/* <div>{ap._score}</div>
        <div>{public_actor_id}</div>
        <div>{public_actor_id_siren}</div> */}
        {startups && <div>Ils ont travaillé avec {startups?.join(',')}</div>}
        {decpFirst.length  ? <><b style={{color: cardType.color}}>{decpFirst.length}</b> Marchés passés</> : null}
        {(decpFirst.length && approchFirst.length) ? " et " : null}
        {approchFirst.length  ? <><b style={{color: cardType.color}}>{approchFirst.length}</b> Achats programmés</> : null}
        {(decpFirst.length || approchFirst.length) ? <div>contiennent des mots de votre recherche</div> : null}
    </ResultCard >
}
