...
export const InvestisseurSearchPage = () => {
  return (
    <SearchPage usedAdvancedFilter={useAdvancedFilters(investisseur.name)} cardType={investisseur}>
      {(card, i, isLoading) => (
        <InvestisseurResultCard key={i} isLoading={isLoading} invest={card as Investisseur} />
      )}
    </SearchPage>
  );
};
...
export const InvestisseurResultCard: React.FC<InvestisseurResultCardProps> = ({
  isLoading,
  invest
}) => {
  ...
  return (
    <ResultCard
      ...
      name={invest['Nom du fonds']}
      toprow={invest['Vous êtes']}
      linkData={invest}
      slug={slug}
      isLoading={isLoading}
    >
      ...
      <p
        ...
        title={invest["Présentation de la politique d'investissement"]}
      >
