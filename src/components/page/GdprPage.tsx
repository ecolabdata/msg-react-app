export const GdprPage = () => {
  return (
    <div className="mx-8 w-4/5">
      <h1 className="my-4 font-bold text-3xl">Données personnelles</h1>
      <p>
        Le Commissariat Général au Développement durable, situé 1 place Carpeaux 92800 à Puteaux,
        s’engage à traiter vos données personnelles en toute confidentialité, dans le respect de la
        réglementation et notamment du Règlement Général sur la Protection des Données 2016/679 et
        de la Loi Informatique et Libertés n°78-17 mise à jour.
      </p>
      ...
      <ul>
        ...
        <li>
          adresses de courrier électronique (e-mail) pour vérifier l’exactitude de l’adresse et
          échanger &nbsp;;
        </li>
      ...
      <h3 className="my-4 font-bold text-xl">Formulaire de création de compte</h3>
      <p>Pas encore de compte utilisateur évolution à venir à moyen ou long terme</p>
      <h3 className="my-4 font-bold text-xl">Comment exercer mes droits ?</h3>
      <p>
        Pour exercer vos droits ou pour toute question sur le traitement de vos données, vous pouvez
        contacter notre Délégué à la Protection des Données à cette adresse :{' '}
      ...
      <p>
        La CNIL définit les cookies comme « un petit fichier stocké par un serveur dans le terminal
        (ordinateur, téléphone, etc.) d’un utilisateur et associé à un domaine web (c'est-à-dire
        dans la majorité des cas à l’ensemble des pages d’un même site web). Ce fichier est
        automatiquement renvoyé lors de contacts ultérieurs avec le même domaine ».
      </p>
      <p>Ce site peut être amené à utiliser deux types de cookies :</p>
      <p>
        les cookies internes pour des fonctionnalités de mesure d’audience et de bon fonctionnement
        du site,
      </p>
      <p>
        les cookies tiers, déposés pour accéder à des fonctionnalités proposées par des services
        tiers.
      </p>
      <p>
        À tout moment, vous pouvez consulter plus d’informations et paramétrer vos choix en matière
        de cookies en vous rendant sur la boîte de dialogue modale de gestion des cookies.
      </p>
    </div>
  );
};
