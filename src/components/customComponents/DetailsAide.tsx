...
const CardDetails: React.FC<{ cardType: CardType }> = ({ cardType }) => {
  ...
  return (
    <>
      <div
        style={{
          marginLeft: 'calc(max(10% - 100px, 0px))',
          marginRight: 'calc(max(10% - 100px, min(8vw, 50px)))'
        }}
        ...
        <div className="cardTitle my-10 text-base   mx-auto">
          ...
          <div className="flex mr-[50px]">
            <div className="flex justify-start flex-1 w-1/2">
              <p style={cardTypeColor} className="mt-6 w-fit text-base">
            ...
            <div className="flex justify-end flex-1 w-1/2">
              <p style={{ opacity: 0.6 }} className="mt-6 w-fit text-base font-thin">
                Source:{' '}
                <a
                  href={'https://aides-territoires.beta.gouv.fr' + cardData['url']}
                  target="_blank"
                  rel="noreferrer"
                >
                  Aides territoires
      ...
      <div
        style={{
          marginRight: 'calc(max(10% - 100px, 0px))',
          marginLeft: 'calc(max(10% - 100px, 0px))'
        }}
        className="flex justify-around flex-wrap flex-row-reverse"
      >
        <div
          style={{ flex: '0 0 380px', ...cardTypeColor }}
          onClick={() => {
        ...
        >
          <h2 style={cardTypeColor} className="text-[22px] font-bold  ">
            Contact
          </h2>

          <p className="text-base w-[280px]">
            {' '}
            <span dangerouslySetInnerHTML={{ __html: cardData['contact'] + '' }}></span>
          </p>
        ...
        <div
          ...
          className="contentContainer flex flex-wrap justify-left mx-[5vw] "
        >
          <SmallFields color={cardType.color} fieldname={'Calendrier'}>
            Date de clôture:{' '}
            {cardData.submission_deadline ? `${displayabeSubmissionDeadLine}` : 'Aide permanente'}
          </SmallFields>
          <SmallFields color={cardType.color} fieldname={'Bénéficiaires'}>
            <ul>
              {cardData.targeted_audiences?.map((x) => (
                <li key={x}>{x}</li>
              ))}
            </ul>
          </SmallFields>
          ...
          <BigFields color={cardType.color} fieldname="Critères d’éligibilité">
            <span
              ...
              dangerouslySetInnerHTML={{ __html: cardData['eligibility'] + '' }}
            ></span>
          </BigFields>
          ...
          {subvention.display && (
            <SmallFields color={cardType.color} fieldname={'Taux de subvention'}>
              {subvention.txt}
            </SmallFields>
          )}
        </div>
...
const SmallFields: React.FC<{ color: string; fieldname: ReactNode; children: ReactNode }> = ({
  color,
  fieldname,
  children
}) => (
  <div style={{ flex: '1 1 auto' }} className="short-field mr-[5vw] mt-8">
    <div style={{ color }} className="font-[700] text-[22px]">
      {fieldname}
    </div>
    <div className="mt-2">{children}</div>
    <div
      className="mt-6"
      style={{ width: '74px', borderTop: '1px solid rgba(206, 206, 206, 0.2)' }}
    ></div>
  </div>
);

const BigFields: React.FC<{ color: string; fieldname: ReactNode; children: ReactNode }> = ({
  color,
  fieldname,
  children
}) => {
  ...
  return (
    <div style={{ flex: '1 1 100%', background: '#353434' }} className="short-field p-[18px] mt-8">
      <div style={{ color }} className="font-[700] text-[22px]">
        {fieldname}
      </div>
      <div className={`mt-2 ${maxHeight} overflow-hidden relative transition-all ease`}>
        ...
        {!showAll && (
          <div
            ...
            style={{ background: 'linear-gradient(180deg, rgba(0,0,0,0) 50%, #353434 90%)' }}
          ></div>
        )}
      </div>
      <button
        style={{ '--border-action-high-blue-france': color, color: color } as CSSProperties}
        className="fr-btn fr-btn--secondary mt-2"
