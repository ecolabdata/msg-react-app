...
const Details: React.FC<DetailsStartupProps> = ({ cardType }) => {
  ...
  const cardPublicMarkets = cardData['Références publiques']
    ? cardData['Références publiques'].split(',')
    : [];
  ...
  const cardProjectSmallDevices =
    cardData['Le projet'] && cardData['Le projet'].substring(0, 453).padEnd(456, '.');
  const cardProjectBigDevices =
    cardData['Le projet'] && cardData['Le projet'].substring(0, 686).padEnd(689, '.');
  ...
  return (
    <div className="globalContainer flex flex-col justify-around">
      ...
      <div
        className="contactAndDescription relative mx-auto max-h-[900px] h-full w-[85%] flex flex-col justify-evenly 
                xl:h-60 xl:flex-row xl:justify-between"
      >
        <div
          className="leftSide order-2 flex flex-col justify-around p-1
        ...
        >
          ...
          >
            <h3 style={cardTypeColor} className="font-bold text-xl">
              Pitch
            </h3>
          ...
          >
            ...
            <div className="mt-2 -ml-[10px] w-full flex flex-wrap">
              {cardSoutiens.length > 0
                ? cardSoutiens.map((soutien) => (
                    <Label bgColor="#E5FFF4" textColor="text-[#37635F]" key={soutien}>
                      {soutien}
                    </Label>
                  ))
                : null}
          ...
          >
            ...
            <div className="mt-2 -ml-[10px] w-full flex flex-wrap">
              {cardPublicMarkets.length > 0
                ? cardPublicMarkets.map((publicMarket) => (
                    <Label bgColor="#E5FFF4" textColor="text-[#37635F]" key={publicMarket}>
                      {publicMarket}
                    </Label>
                  ))
                : null}
        ...
        <div
          className="contact rounded-sm w-96 h-[230px] flex flex-col items-center justify-evenly addBorder border-2 border-[#4EC8AE]
                xl:order-2 xl:my-auto xl:p-6"
        >
          <h3 style={cardTypeColor} className="font-bold text-2xl">
            {' '}
            Contacts et détails{' '}
          </h3>
          ...
          {!showContactDetails ? (
            <>
              <button
                onClick={() => handleClickOnContactDetails()}
                type="button"
                className="w-fit text-base text-left py-2 px-6 flex justify-between text-[#3A3A3A] bg-[#4EC8AE]"
              >
            ...
            <div className="">
              <span className="text-base" style={cardTypeColor}>
                {cardData.Téléphone}
              </span>{' '}
              <br />
              <span className="text-base" style={cardTypeColor}>
      ...
      <div className="contentContainer mx-auto w-[85%]">
        <div
          className="enterprise -ml-4 mt-8 flex flex-col justify-around
                xl:mt-0 xl:w-[60%]"
        >
          <h3 style={cardTypeColor} className="font-bold text-xl m-4">
            Entreprise
          </h3>
        ...
        <div
          className={`project relative mt-12 p-3.5 bg-[#353434]
                    xl:w-[60%]
        ...
        >
          ...
          >
            ...
            {!seeEntireProjectDescription ? (
              <>
                {screenWidth >= 1280 ? (
                  <p className="m-4">{cardProjectBigDevices}</p>
                ) : (
                  <p className="m-4">{cardProjectSmallDevices}</p>
                )}
          ...
          <button
            onClick={() => setSeeEntireProjectDescription(!seeEntireProjectDescription)}
            style={cardTypeColor}
            className="relative m-4 w-fit addBorder border text-sm p-2"
            type="button"
          >
        ...
        <div
          className="marketsAndChallenges mt-12 flex flex-wrap justify-around 
                xl:w-[60%]"
        >
          <div
            className="markets mt-4 w-full flex flex-col 
            ...
            <div className="mt-2 -ml-[10px] w-full flex flex-wrap justify-start ">
              {cardMarkets.length > 0
                ? cardMarkets.map((market) => (
                    <Label bgColor="#E5FFF4" textColor="text-[#37635F]" key={market}>
                      {market}
                    </Label>
                  ))
                : null}
          ...
          <div
            className="challenges mt-4 w-full flex flex-col 
                    xl:min-w-[50%] xl:max-w-[50%] xl:max-h-[50%]"
          >
            <h3 style={cardTypeColor} className="font-bold text-xl">
              Enjeux ODD
            ...
            <div className="mt-2 -ml-[10px] w-full flex flex-wrap justify-start">
              {cardChallenges.length > 0
                ? cardChallenges.map((challenge) => (
                    <Label bgColor="#E5FFF4" textColor="text-[#37635F]" key={challenge}>
                      {challenge}
                    </Label>
                  ))
                : null}
        ...
        <p className="mt-16">
          Source de la donnée : <a href="#annuaur Greentech Innovation"> Annuaire Greentech</a>{' '}
        </p>

        <p className="mb-5 mt-10">Partager la page</p>

        <div className="shareContainer relative share mb-16 -ml-6 w-64 flex justify-evenly">
          <a className="rm-link-underline " href="Facebook" title="redirection vers Facebook">
            <span
              className="fr-fi-facebook-circle-line text-dark-text-action hover:"
              aria-hidden="true"
            />
          </a>
          <a className="rm-link-underline " href="Twitter" title="redirection vers Twitter">
            <span className="fr-fi-twitter-line text-dark-text-action" aria-hidden="true" />
          </a>
          <a className="rm-link-underline " href="LinkedIn" title="redirection vers Linkedin">
            <span className="fr-fi-linkedin-box-line text-dark-text-action" aria-hidden="true" />
          </a>
          <a
            className="rm-link-underline "
            href="mailto:msg@greentech.fr"
            title="redirection vers la boîte mail"
          >
            <span className="fr-fi-mail-line text-dark-text-action" aria-hidden="true" />
          </a>
          <span
