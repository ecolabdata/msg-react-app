...
import { Aide, AidesQuery, searchAidesClient, searchAidesInno } from '../../../api/Api';
...
export class AideRequestFilter implements RequestFilter {
  ...
  constructor(
    ...
    if (initialState?.search.cards) {
      ...
      this.allCards = this.filter(
        cardType.name === 'aides-innovations'
          ? initialState.search.cards.aides_innovation
          : initialState.search.cards.aides_clients
      );
    }
  }
  ...
  filter(cards: Aide[]) {
    ...
    if (echeance) {
      cards = cards.filter((card) => {
        if (card.name === "Réaliser des projets de solidarité internationale d'accès à l'eau")
  ...
  Component = ({}) => {
    ...
    return (
      <>
        <Select
          classes=" w-[93%] 
                                    lg:mx-2 lg:max-w-[202px]"
          label="Nature de l'aide"
          color={cardType.color}
          defaultOption={'Toutes'}
          optionsData={all_aides_types.results.map((x) => x.name)}
          onChange={(e) => {
        ...
        <Select
          ...
          color={cardType.color}
          defaultOption={'Toutes'}
          optionsData={Object.keys(echeances)}
          onChange={(e) => {
            this.echeance = e.currentTarget.value;
            setEcheance(e.currentTarget.value);
          }}
