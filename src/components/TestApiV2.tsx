...
const TestApiV2 = ({}) => {
  ...
  useEffect(() => {
    if (query) {
      fetch(
        'https://api-v2.msg.greentechinnovation.fr/acteur_public/search?' +
          new URLSearchParams({
            q: query
          })
      )
  ...
  const respHtml = useMemo(() => {
    ...
    return resp.hits.map((hit: any) => {
      ...
      const glHtml = Object.keys(hit.highlight).map((fieldname) => (
        <div key={fieldname}>
          <div className="brightness-75">{fieldname} :</div>
          <p
            style={{ width: '50%' }}
            dangerouslySetInnerHTML={{ __html: replaceHlTxt(hit.highlight[fieldname][0]) }}
          ></p>
        </div>
      ));
      return (
        <div className={'mt-10'} key={nomHtml}>
          <h1>{nomHtml}</h1>
          <div>{glHtml}</div>
        </div>
      );
  ...
  return (
    <div>
      ...
      />
      <div>{respHtml || 'Pas de réponses'}</div>
    </div>
  );
};
