...
const useJwtAuth = (
  noToken: () => void,
  invalidToken: () => void,
  validToken: (payload: JwtPayload) => void
) => {
...
export const JwtAuthProvider = ({ children }: Props) => {
  const [authMsg, setAuthMsg] = useState<React.ReactElement>(
    <JwtStateContext.Provider value={{ name: 'checking' }}>{children}</JwtStateContext.Provider>
  );
  const provideState = (state: JwtState) =>
    setAuthMsg(<JwtStateContext.Provider value={state}>{children}</JwtStateContext.Provider>);
  useJwtAuth(
